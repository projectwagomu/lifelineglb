...
public class StartNQueens {
	...
	public static void main(String[] args) {
		...
		final int threshold = Integer.parseInt(cmd.getOptionValue("t", String.valueOf(THRESHOLD_DEFAULT)));
		final int qSize = Integer.parseInt(cmd.getOptionValue("q", String.valueOf(QSIZE_DEFAULT)));
		System.out.println("NQueens config:\n" + "  queens=" + queens + "\n" + "  threshold=" + threshold + "\n"
				+ "  qSize=" + qSize + "\n");
		...
		for (int i = 0; i < repetitions; i++) {
			...
			final GLBcomputer<LongSum, NQueens> glb = new GLBFactory<LongSum, NQueens>().setupGLB(places());

			final int _queens = queens;
			final int _threshold = threshold;
			final int _qSize = qSize;
			final LongSum sum = glb.computeDynamic(nQueens, () -> new LongSum(0),
					() -> new NQueens(_queens, _threshold, _qSize));
