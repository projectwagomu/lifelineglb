...
public class X10Random {
	...
	private static long mixGamma(long z) {
		...
		final long n = Long.bitCount(z ^ (z >>> 1));
		if (n >= 24) {
	...
	public X10Random() {
		final long s = defaultGen.getAndAdd(2 * GOLDEN_GAMMA);
		seed = mix64(s);
	...
	/**
	 * Return a 32-bit random integer in the range 0 to maxPlus1-1 when maxPlus1 >
	 * 0. Return 0 if maxPlus1 <= 0 instead of throwing an IllegalArgumentException,
