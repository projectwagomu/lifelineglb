 ...
 * nodes explored, using the {@link LongSum} class.
 *
...
public class MultiworkerUTS implements Bag<MultiworkerUTS, LongSum>, Serializable {
	...
	/**
	 * {@link MessageDigest} instance held by this {@link Bag} instance. This member
	 * is set to transient so as not to be serialized when instances of this class
	 * are transfered from one place to another to perform load balance.
	 */
	 ...
	 * node have been explored, the value in the {@link #upper} array is decremented
	 * and the next node is explored (provided {@link #lower} at that index is
	 * inferior to {@link #upper}).
	 */
	int[] upper;
	 ...
	 * @throws DigestException if the {@link MessageDigest} throws an exception when
	 *                         called
	 */
	 ...
	 * @throws DigestException if the provided {@link MessageDigest} throws an
	 *                         exception
	 */
	...
	/**
	 * Indicates if the DepthFirstSearch exploration of the tree can be split. This
	 * criteria is deemed satisfactorily met when at a certain point in the current
	 * branch exploration, there remains at least 2 leaves, that is is the
	 * difference between {@link #lower} and {@link #upper} at a certain index is
	...
	/**
	 * Stitches the given tree exploration to the current state. The arrays used to
	 * contain the tree exploration information will increase in size to accommodate
	 * the given tree if necessary.
	 */
	@Override
	...
	/**
	 * Performs node exploration until either the "work-amount" of nodes is explored
	 * or the tree exploration is finished. The second parameter is unused. There is
	 * no requirement for workers to share information during the tree exploration.
	 * The parameter is present to match the computation abstraction of the
	 * multi-worker GLB design which allows this possibility
	 */
	@Override
	...
	/**
	 * Plants the seed of the tree. Needs to be called before the tree exploration
	 * is started.
	 *
	...
	/**
	 * Splits the tree exploration by giving half of the leaves remaining to explore
	 * to an instance which is then returned.
	 */
	@Override
	 ...
	 * {@link #process(int, LongSum)} method by this instance into the given
	 * {@link LongSum}.
	 */
	@Override
