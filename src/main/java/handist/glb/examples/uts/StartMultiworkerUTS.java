...
public class StartMultiworkerUTS {
	...
	public static void main(String[] args) {
		...
		final int branching = Integer.parseInt(cmd.getOptionValue("b", String.valueOf(BRANCHING_DEFAULT)));
		...
		for (int i = 0; i < repetitions; i++) {
			...
			final GLBcomputer<LongSum, MultiworkerUTS> glb = new GLBFactory<LongSum, MultiworkerUTS>()
					.setupGLB(places());

			final int _qSize = qSize;
			final LongSum sum = glb.computeDynamic(multiworkerUTS, () -> new LongSum(0),
					() -> new MultiworkerUTS(_qSize));
