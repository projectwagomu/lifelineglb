/**
 * This file is part of the Handy Tools for Distributed Computing project HanDist
 * (https://github.com/handist)
 *
 * <p>This file is licensed to You under the Eclipse Public License (EPL); You may not use this file
 * except in compliance with the License. You may obtain a copy of the License at
 * http://www.opensource.org/licenses/eclipse-1.0.php
 *
 * <p>(C) copyright CS29 Fine 2018-2019.
 */
 ...
 * </pre>
 *
 * @param <B> Type parameter used for the return type of method
 ...
 * @param <R> Type parameter used for method {@link #submit(Fold)}. The chosen
...
public interface Bag<B extends Bag<B, R> & Serializable, R extends Fold<R> & Serializable> {
	 ...
	 * {@link #split(boolean)} and given to an other worker for computation without
	 * depleting the instance of all its work. If this bag is empty, this method
	 ...
	 * parameter. If there is less computation in the {@link Bag} then the requested
	 * amount of work, should complete all the tasks and then return.
	 *
	 * <p>
	 * The second parameter is the object shared with the other local concurrent
	 * workers. Information can be shared between workers through this instance. The
	 * programmer will be careful to enforce proper synchronization on that shared
	 * object where it is necessary, the library does not enforce any particular
	 * protection on that object.
	 *
	 * @param workAmount   the amount of computation to be done
	 * @param sharedObject instance shared between workers of one place through
	 *                     which some data between workers can be shared
	 * @return the number of processed tasks
	 */
	public int process(int workAmount, R sharedObject);

	/**
	 * Takes a chunk of computation from this bag and returns it in a new instance.
	 * The computation fragment this method returns will be computed by a different
	 * thread, possibly on a different host.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If this method is called when this instance is empty, should return an empty
	 * bag instance.
	 *
	 * @param takeAll indicates if the caller wants the whole content of this
	 *                instance in the event it cannot be split. Can be ignored by
	 *                the programmer in other situations.
	 * @return a fragment of the computation held in this bag in a new instance
	 */
	public B split(boolean takeAll);

	/**
	 * Asks for the result produced by this fragment of the computation to be placed
	 * in the given result R instance.
	 *
