...
public class GLBcomputer<R extends Fold<R> & Serializable, B extends Bag<B, R> & Serializable> extends PlaceLocalObject
	...
	/**
	 * {@link Logger} instance used to gather the {@link PlaceLogger}s of each place
	 * and hold runtime information common to the whole computation.
	 *
	 * @see #getLog()
	 */
	 ...
	 * the array). A {@code 1} value at index {@code i} indicates that the "i"th
	 * worker is requested to send work to the {@link #interPlaceQueue}. The whole
	 * array is turned to 1 values when it is discovered by a worker that the
	 * {@link #interPlaceQueue} is empty. Workers put their assigned flag back to 0
	...
	/**
	 * Lock used by the {@link #lifelineAnswerThread()} to yield its thread. When a
	 * lifeline answer becomes possible, a {@link #workerProcess(WorkerBag)} will
	 * {@link Lock#unblock()} this lock to allow progress of the
	 ...
	 * thread is woken up for exit. When the lifeline answer calls proceeds to
	 * method {@link #run(Bag)}, it can put {@link #shutdown} back to {@code false}
	 * before the old thread could exit, resulting in multiple lifeline answer
	 * thread running on the same place.
	 *
	 * <p>
	 * To solve this issue, method {@link #run(Bag)} actively waits until this flag
	 * is set back to true by the exiting lifeline answer thread before spawning a
	 * new one.
	 */
	 ...
	 * method {@link #steal(int, GlobalRef)}. The thieves stored in this member will
	 * be answered by the thread running the {@link #lifelineAnswerThread()} when
	 ...
	 * answer lifelines. When a {@link #workerProcess(WorkerBag)} decided to unlock
	 * the progress of the lifeline answer thread ({@link #lifelineAnswerThread()}),
	 * it sets the value of {@link #lifelineToAnswer} to {@code true}. The value is
	 * set back to {@code false} by the {@link #lifelineAnswerThread()} when it
	 * actually becomes active again.
	 *
	 * <p>
	 * Worker processes check the value of this boolean before considering waking
	 * the lifeline answer thread. This avoids having multiple workers repetitively
	 * attempting to wake up the lifeline answer thread.
	 * {@link #lifelineAnswerThread()}.
	 */
	 ...
	 * {@link #getLog()}. Avoids a second aggregation of the logs if multiple calls
	 ...
	 * {@link #workerProcess(WorkerBag)} activities to check if the pool has pending
	 * "shorter" activities and yield if necessary.
	 *
	 * <p>
	 ...
	 * avoids having concurrent workers attempting to yield using the same lock. In
	 * practice, the current implementation has a single lock: {@link #workerLock},
	 ...
	 * concurrent workers ({@link GLBMultiWorkerConfiguration}) are placed in this
	 * collection as part of method
	 * {@link #reset(SerializableSupplier, SerializableSupplier, SerializableSupplier, boolean, List, boolean)}.
	 *
	 * <p>
	 * <em>During the computation</em>, this collection contains the {@link Bag}s of
	 * the workers that are not active. Prior to launching a new asynchronous
	 ...
	 * fraction of the computation and could not get more work from the intra place
	 * load balancing mechanisms, it places its {@link WorkerBag} back into this
	 * collection.
	 *
	 * <p>
	 * This collection is also used as the lock provider for synchronized blocks
	 * when member {@link #workerCount} needs to be read or modified in a protected
	 * manner. This includes segments of methods {@link #deal(int, Bag, GlobalRef)}
	 * and {@link #workerProcess(WorkerBag)}.
	 *
	 * <p>
	 * <em>After the computation</em>, all the {@link Bag}s processed by the workers
	 * are present in this collection. This allows access for the collection of each
	 ...
	 *
	 * <p>
	 * Note that the value carried by this variable can be different than the actual
	 * number of workers working concurrently. For instance, when a new asynchronous
	 * {@link #workerProcess(WorkerBag)} needs to be launched, this variable is
	 * incremented <em>before</em> the asynchronous process is launched. Moreover,
	 * the {@link #workerProcess(WorkerBag)} can cooperatively yield its thread
	 * usage to allow some other asynchronous activities to be performed. Those
	 * yields do not change the value of {@link #workerCount} but are tracked
	 * separately in class member {@link #logger} with methods
	...
	GLBcomputer() {
		...
		try {
			ls = (LifelineStrategy) Class
					.forName(GLBMultiWorkerConfiguration.GLBOPTION_MULTIWORKER_LIFELINESTRATEGY.get())
		...
		feedInterQueueRequested = new AtomicIntegerArray(
	 ...
	 * asynchronously, this method will block until all places have completed their
	 * {@link #collectResult} method.
	 */
	...
	/**
	 * Makes the place gather the results contained by all its bags into its member
	 * {@link #result}. If this place ({@link #HOME}) is not place 0, sends the
	 * content of the result to place 0 to be merged with all the other results
	 * there.
	 */
	void collectResult() {
	 ...
	 * @param emptyBagSupplier   function that provides new empty computation bag
	 *                           instances
	 * @return aggregated result of the computation
	 */
	public R computeDynamic(B bag, SerializableSupplier<R> initResultSupplier,
	 ...
	 * @param queueInitializer  initializer for the queue used for load balancing
	 *                          purposes
	 ...
	 *         computation
	 */
	public R computeDynamic(B work, SerializableSupplier<R> resultInitializer, SerializableSupplier<B> queueInitializer,
	...
	public R computeStatic(SerializableSupplier<R> resultInitializer, SerializableSupplier<B> queueInitializer,
			SerializableSupplier<B> workerInitializer) {
		// We reset every place
		final long initStart = System.nanoTime();
		this.resultInitializer = resultInitializer;
	...
	/**
	 * Method called on this place when a victim of steal is answering and providing
	 * some loot.
	 *
	 * <p>
	 ...
	 * <li>If no workers exist and the place is performing some steals, the loot is
	 * placed in the first workerBag of collection {@link #workerBags} before
	 ...
	 * </ul>
	 *
	 * @param victim    the id from place sending the loot or {@code -1} if it is a
	 *                  random steal
		...
		do {
			...
			synchronized (lifelineLock) {
				...
				while (!lifelineThieves.isEmpty()) {
					...
					synchronized (intraPlaceQueue) {
						...
						console.println(
								"BEFORE performing interPlaceQueue.split(true) and interPlaceQueue should now contain tasks empty, interPlaceQueue.isEmpty="
										+ interQueueEmpty + ", interPlaceQueue.size()="
										+ interPlaceQueue.getCurrentTaskCount() + " loot.size()=0");
						...
						console.println(
								"AFTER performing interPlaceQueue.split(true) and interPlaceQueue should now be empty, interPlaceQueue.isEmpty="
										+ interQueueEmpty + ", interPlaceQueue.size()="
										+ interPlaceQueue.getCurrentTaskCount() + " loot.size()="
										+ loot.getCurrentTaskCount());
					}
					// Send the loot
					final int h = HOME.id;
					...
					console.println(
							"sends loot to lifeline=" + lifelineThief + ", loot.size=" + loot.getCurrentTaskCount());
					try {
						asyncAt(place(lifelineThief), () -> {
	 ...
	 * </ul>
	 *
	 * @return {@code true} if some work is received during the method's execution,
		...
		synchronized (lifelineLock) {
			for (int i = 0; i < LIFELINE.length; i++) {
				...
				if (!isLifelineEstablished && isValidRemotePlace(lifelineID)) { // We check if the lifeline was
					...
					console.println("sends steal request to lifeline=" + lifeline + ", workerCount=" + workerCount);
					final GlobalRef<CountDownLatch> waitLatch = new GlobalRef<>(new CountDownLatch(1));
					try {
						uncountedAsyncAt(lifeline, () -> {
							steal(h, waitLatch);
						});
					} catch (final Throwable t) {
						t.printStackTrace(System.out);
	...
	/**
	 * Part of the {@link #run(Bag)} procedure. Performs random steals until one of
	 * two things happen:
	 *
	 * <ul>
			...
			try {
				immediateAsyncAt(newPlace, () -> {
					try {
						...
						newComputer.reset(_resultInitializer, _queueInitializer, _workerInitializer, true, newPlaceIds,
								false);

						immediateAsyncAt(newPlacesCount.home(), () -> {
	...
	/**
	 * After the reduction in the number of running places, nothing in particular
	...
	/**
	 * Nothing in particular needs to be performed before a grow order is put into
	 * place.
	 */
	@Override
	...
	/**
	 * When a shrink order is received, the places chosen to be removed are cut off
	 * from the lifeline network and their work is sent back to remaining places.
	 * Their intermediary result also needs to be transferred back to remaining
	 * places to be merged.
	 * <p>
	 * In this implementation, we always choose the places with the largest IDs to
	 * be released.
	 */
	@Override
	...
	/**
	 * Procedure used to recalculate the lifeline network when places are added
	 * and/or removed from the runtime. This method is called by
	 * {@link #postGrow(int, List, List)} and {@link #preShrink(int)} on each place
	 * that is continuing execution.
	 *
	 * @param addedPlaces places that were added as a result of the previous
	 *                    malleable operation (may be empty)
	 */
	...
	/**
	 * Routine called on the places that will remain with the runtime just before
	 * the shrink is performed. These places will re-compute their lifelines and
	 * discard any lifeline connecting to a place which is about to be removed as
	 * part of the shrink operation
	 *
	 * @param highestID          the largest place ID for the places that remain in
	 *                           the runtime
	 * @param allRemainingPlaces the places that will still be participating in the
	 *                           runtime after the shrink operation
	 * @param removedPlaces      the list of places to be removed from the runtime
	 */
	private void recalculateLifelinesBeforeShrink(int highestID, List<Place> allRemainingPlaces,
		...
		synchronized (lifelineLock) {
			...
			for (final Place p : removedPlaces) {
				...
				lifelineEstablished.put(p.id, true); // hack preventing the re-establishment of a lifeline on a place
	...
	private void resetAll(final boolean staticTasks) {
		...
		try {
			finish(() -> {
				for (final Place p : places()) {
					asyncAt(p, () -> reset(_resultInitializer, _queueInitializer, _workerInitializer, false, null,
							staticTasks));
				}
			});
		} catch (final Throwable t) {
	 ...
	 *
	 * @param thief     the integer id of the place performing the steal, or `(-id -
	...
	synchronized void steal(int thief, GlobalRef<CountDownLatch> waitLatch) {
		...
		final int h = HOME.id;
		final B loot = loot();
		console.println("received steal request from " + thief + ", loot().size="
				+ (loot == null ? "0" : loot.getCurrentTaskCount()) + ", workerCount=" + workerCount);
		...
		if (thief >= 0) {
			...
			if (loot != null) {
				...
				try {
					uncountedAsyncAt(place(-thief - 1), () -> {
	...
	private void transferWorkBeforeShutdown(ArrayList<Place> toStop, GlobalRef<CountDownLatch> toStopCount) {
		...
		while (myWorkerCount > 0) {
			...
			console.println("synchronized (workerBags): while (this.workerCount > 0): waiting, myWorkerCount="
					+ myWorkerCount + ", workerAvailableLocks.size=" + workerAvailableLocks.size()
		...
		synchronized (workerBags) {
			synchronized (intraPlaceQueue) {
				...
				if (workerBags.size() != worker) {
					console.println("Error: workerBags.size()=" + workerBags.size() + ", but should be " + worker);
				} else {
				...
				for (final WorkerBag wb : workerBags) {
					...
					console.println("merged workbag.id=" + wb.workerId + ", wb.bag.result=" + wb.bag.getResult()
							+ ", taskCount=" + wb.bag.getCurrentTaskCount());
				}
			}
		}
		console.println("dealBag.result=" + dealBag.getResult() + ", taskCount=" + dealBag.getCurrentTaskCount());
		...
		asyncAt(place(target), () -> {
			console.println("(in asyncAt) (before deal), dealBag.result=" + dealBag.getResult() + ", taskCount="
		...
		immediateAsyncAt(toStopCount.home(), () -> {
			computationLog.addPlaceLogger(l);
	 ...
	 * instance to let the Java Virtual Machine perform some optimizations that will
	 * yield better performance when the larger "real" computation is later
	 ...
	 *
	 * @param warmupBagSupplier supplier of the work sample which will be computed
	 *                          at each place independently as a warm-up
	 * @param resultInitializer initializer for the result instance of each place
	 * @param queueInitializer  initializer for the two queues used at each place to
	 ...
	 */
	public Logger warmup(SerializableSupplier<B> warmupBagSupplier, SerializableSupplier<R> resultInitializer,
			SerializableSupplier<B> queueInitializer, SerializableSupplier<B> workerInitializer) {
	 ...
	 *
	 * <ol>
	 * <li>Spawns a new {@link #workerProcess(WorkerBag)} if the bag it is currently
	 * processing can be split and the
	 * {@link GLBMultiWorkerConfiguration#GLBOPTION_MULTIWORKER_WORKERPERPLACE}
	 * limit was not reached (meaning there are {@link Bag} instances left in member
	 * {@link #workerBags})
	 * <li>Checks if the {@link #intraPlaceQueue} bag is empty. If so and the
	 * currently held bag can be split ({@link Bag#isSplittable()}), splits its bag
	 * and merges the split content into {@link #intraPlaceQueue}.
	 ...
	 * {@code true} and this worker can split its bag, the worker sends half of the
	 * work it holds into the {@link #interPlaceQueue}.
	 * <li>Check if there are pending lifeline answers that can be answered. If so,
	 * unblocks the {@link #lifelineAnswerThread()}'s progress by unlocking the
	 * {@link #lifelineAnswerLock}.
	 * <li>If there are activities that are waiting for execution and the number of
	 * active workers has reached the number of available cores on the system,
	 ...
	 * </ol>
	 *
	 * @param workerBag computation to process along with an identifier for this
	 *                  worker process
	 */
			...
			do {
				...
				if (Configuration.CONFIG_APGAS_CONSOLEPRINTER.get()) {
					...
					if (((now - lastPrint[myWorkerID]) / 1e9) > 4) {
						...
						console.println("workerID=" + myWorkerID + ", allProcessedTasks=" + allProcessedTasks
								+ ", bag.getCurrentTaskCount()=" + bag.getCurrentTaskCount() + ", intraPlaceQueue="
								+ intraPlaceQueue.getCurrentTaskCount() + ", interPlaceQueue="
								+ interPlaceQueue.getCurrentTaskCount() + ", workerCount=" + workerCount);
					}
				}
