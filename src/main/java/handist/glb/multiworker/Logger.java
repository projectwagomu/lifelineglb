...
/**
 * Logger class for a distributed computation. Keeps information about the global computation time
 * and contains the specifics of each place. The runtime information of each place are kept in
 * instances of {@link PlaceLogger} class.
 *
...
public class Logger {
  ...
  /**
   * Map containing the {@link PlaceLogger} instance of each place. The key represents the place id
   */
   ...
   *
   * <p>Initializes a Logger instance with the computation and result time specified as parameter.
   * It is assumed the result gathering starts directly after the computation.
   */
  Logger() {
  ...
  /**
   * Adds the given {@link PlaceLogger} instance to the logs of each place. The idle time of the
   * place logger is adjusted to match the total time of the computation (during the computation
   * phase, each place starts slightly after the beginning of the first place as some time is needed
   * for the computation to propagate across all places).
   *
   * @param l the {@link PlaceLogger} instance of a certain place in the computation.
   */
  ...
  /**
   * Displays the computation runtime information on the provided output stream in a <em>CSV</em>
   * format.
   *
    ...
    for (final PlaceLogger l : placeLogs.values()) {
      out.println(
          l.place
              ...
              + l.stealsSuffered
              + ";"
              + l.lifelineStealsAttempted
              + ";"
              + l.lifelineStealsSuccess
              + ";"
              + l.lifelineStealsReceived
              + ";"
  ...
  public void setTimings(
      long initStart, long computationStart, long computationEnd, long resultGatheringEnd) {
