...
public class PlaceLogger implements Serializable {
	 ...
	 *
	 * @param placeId integer identifier of the place this PlaceLogger instance is
	...
	public PlaceLogger(int placeId) {
		...
		timeStealing = new long[GLBMultiWorkerConfiguration.GLBOPTION_MULTIWORKER_WORKERPERPLACE.get() + 1];
	}
	...
	public void print(PrintStream out) {
		...
		out.println("This place succeeded     " + lifelineStealsSuccess + "/" + lifelineStealsAttempted);
		out.println("Other places succeeded   " + lifelineStealsSuffered + "/" + lifelineStealsReceived);
	...
	/**
	 * Signals that a worker has ran out of work and will now attempt to steal from
	 * the shared queue.
	 */
	...
	/**
	 * Method called when a worker starts yielding to allow for other activities to
	 * be run by the place.
	 */
