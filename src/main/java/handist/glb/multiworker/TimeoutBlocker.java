...
/**
 * {@link ManagedBlocker} implementation that blocks a thread in the {@link ForkJoinPool} used by
 * the APGAS runtime until a certain moment in time. This class allows for a thread of a {@link
 * ForkJoinPool} to be blocked for most of the time and only "wake-up" to perform some short
 * periodic task.
 *
 * <p>The tuning mechanism as well as the whisperer mechanism rely on this class to be called
 * regularly throughout the computation.
 *
...
public class TimeoutBlocker implements Serializable, ManagedBlocker {
  ...
  /**
   * Member used to circumvent the timeout and make the thread available for computation
   * immediately.
   */
  ...
  /**
   * Sets the next wake-up stamp until which the thread that may use this blocker will be blocked.
   * The thread will be relased when the result of method {@link System#nanoTime()}
   *
